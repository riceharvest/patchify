...
# Usage: irm https://raw.githubusercontent.com/riceharvest/agentic-edit/main/install.ps1 | iex
# Override version: $env:AGENTIC_EDIT_VERSION = "v0.1.0"
$ErrorActionPreference = "Stop"

$Repo = "riceharvest/agentic-edit"
$Bin = "agentic-edit"
$InstallDir = if ($env:AGENTIC_EDIT_INSTALL_DIR) { $env:AGENTIC_EDIT_INSTALL_DIR } else { "$env:USERPROFILE\.local\bin" }

function Fail($message) {
    Write-Error "agentic-edit-installer: ERROR: $message"
    exit 1
}
...
# Version resolution.
if ($env:AGENTIC_EDIT_VERSION) {
    $Version = $env:AGENTIC_EDIT_VERSION
} else {
    Write-Host "agentic-edit-installer: resolving latest release..."
    try {
        $Release = Invoke-RestMethod -Uri "https://api.github.com/repos/$Repo/releases/latest" -ErrorAction Stop
        $Version = $Release.tag_name
    } catch {
        Fail "could not determine latest release (set `$env:AGENTIC_EDIT_VERSION to pin one): $_"
    }
}
...
try {
    Write-Host "agentic-edit-installer: downloading $Version for $Target..."
    ...
    Write-Host "agentic-edit-installer: verifying checksum..."
    ...
    Write-Host "agentic-edit-installer: extracting..."
    ...
    Write-Host "agentic-edit-installer: installed $Version to $InstallDir\$Bin.exe"
    if ($env:PATH -notlike "*$InstallDir*") {
        Write-Host "agentic-edit-installer: NOTE: $InstallDir is not on your PATH"
    }
} finally {
